...
valid_volume = {'axial', 'sagittal', 'coronal', 'quit'};
valid_noise = {'salt & pepper', 'gaussian', 'none'};
while 1
    % Requesting user to choose type of slice
    volume_type = input('Write the plane of interest ("quit" to quit): ', 's');
    ...
    % Requesting user to choose type of noise
    noise_type = input('Add noise ("none" if none): ', 's');
    while ~any(strcmp(noise_type, valid_noise))
        noise_type = input('Write the plane of interest: ', 's');
    end 
    if strcmp(noise_type, 'none')
        chosen_vol = vol;
    else
        % TODO: Make noise degree iterable (now fixed to 0.05)

        % Supposing adding the noise is independent from slice type
        chosen_vol = addnoise(vol, noise_type, 0.05);
    end

    % TODO: Calling different segmentation functions in the if else or seg3
    % ulterior parameter with noise so as to apply proper filter
