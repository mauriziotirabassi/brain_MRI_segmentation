...
sg_planes=rot90(permute(squeeze(vol(100:145,:,:)),[2,3,1])); %46 slices
ax_planes=rot90(squeeze(vol(:,:,60:90))); %31 slices
cr_planes=rot90(permute(squeeze(vol(:,140:180,:)),[1,3,2])); %41 slices
sg=rot90(permute(squeeze(vol(100:145,140:180,60:90)),[2,3,1])); %46 slices
ax=rot90(squeeze(vol(100:145,140:180,60:90))); %31 slices
cr=rot90(permute(squeeze(vol(100:145,140:180,60:90)),[1,3,2])); %41 slices
...
for j=0.1:0.1:0.8
    ...
    for i=1:size(sg,3)
        for gamma=0.1:0.1:2
        tmp=imadjust(sg(:,:,i),[0 1], [0 1], gamma);
...
for i=1:size(ax,3)
    for gamma=0.1:0.1:2
    tmp=imadjust(ax(:,:,i),[0 1], [0 1], gamma);
    tmp=imbinarize(tmp,graythresh(tmp));
    D(i,int8(gamma*10))=dice(rot90(squeeze(true_seg(:,:,i))),tmp);
    end
end
...
for i=1:size(cr,3)
    for gamma=0.1:0.1:2
    tmp=imadjust(cr(:,:,i),[0 1], [0 1], gamma);
    tmp=imbinarize(tmp,graythresh(tmp));
    D(i,int8(gamma*10))=dice(rot90(squeeze(true_seg(:,i,:))),tmp);
    end
end
...
for j=0.1:0.1:0.8
    ...
    for i=1:size(sg,3)
        for gamma=0.1:0.1:2
        tmp=imadjust(squeeze(sg(:,:,i)),[0 1], [0 1], gamma);
